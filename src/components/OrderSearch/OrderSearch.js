  ...
  * */
  fiveCategoryDeal = (params) => {
    ...
    this.state.needFilter.map((filter) => {
      if (filter.key === 'lobBrandThreeType') {
        filter.value = (<div style={{ margin: '.8rem 0 2.6rem 2rem' }}><FiveCategorySelector
          params={params}
          callback={this.fiveCategoryDeal} authorizedOrganizationId={this.props.globalStore.organizationId}
        /></div>);
      }
    });
  }
  ...
  searchClick = () => {
    /* {
     "SPFLID": 1, //商品分类ID
     "SPBH": "2131231", //商品编号
     "SL": 1, //数量，为1表示需要增加筛选条件：库存数量不为0， 不需要该条件则不传
     "WQ": { //物权
     "WDKC": 1,  //我的库存
     "PJLDH": 1, //铺进来的货
     "PCQDH": 1 //铺出去的货
     },
     "BZ": {  //包装, 勾选全部（消毒包装+非消毒包装）时不传
     "YXQZ":{  //有效期止 ,选择非消毒包装时不传， 勾选全部效期不需要传里面的起始时间和截止时间
     "QSSJ": "1000000000",  //起始时间
     "JZSJ": "1000000000"   //截止时间，勾选“一年以上”选项时不传
     }
     }
     }*/
    let params = { SL: 1 };
    ...
    if (this.state.filterResult.lobBrandThreeType) {
      ...
      if (this.state.filterResult.lobBrandThreeType.selectBrandId && this.state.filterResult.lobBrandThreeType.selectBrandId !== -1) {
        params = { ...params, SPPPID: this.state.filterResult.lobBrandThreeType.selectBrandId };
      }
  ...
  radioFilterDeal = opData => (event, value) => {
    ...
    console.log(' this.filterResult ===', this.state.filterResult);
    this.setState({
      filterResult: this.state.filterResult,
      [opData.key]: value
    })
  ...
  getSecondFilter = (opData) => {
    ...
    if (opData.type === 1) {
      filterArr = opData.data.map((valueData, valueIndex) => <li key={`filtrateOptions${valueIndex}`}>
        <label><input type='checkbox' className='optionsFiltrateCheckbox' value={valueData.value} checked={valueData.checkState} onClick={this.checkboxFiltrateOptions(this.state.optionsData.key, valueData)} /><i>✓</i>{valueData.filtrateVal}</label>
      </li>);
      ...
      return (<RadioButtonGroup
        ...
        style={{ margin: '.8rem 0 2.6rem 2rem' }}
        name={opData.key}
      ...
      console.log('..............type=3', opData.data)
      return opData.data;
    }
  }
  componentWillMount() {
              ...
              <ul>
                {
                  this.state.needFilter.map((data, index) => <li key={`filtrateTyple${index}`} onClick={this.handleFiltrateTyple(data)} style={{ backgroundColor: data.isShowColor ? '#00A0FF' : '#fff', color: data.isShowColor ? '#fff' : '#00A0FF' }}>
                    <div className='filtrateCondition' style={{ color: data.isShowColor ? '#fff' : '#00A0FF' }}>
                      <label><input type='checkbox' className='filtrateCheckbox' checked={data.isShowChecked ? 'true' : 'false'} /><i style={{ visibility: data.isShowChecked ? 'visible' : 'hidden' }}>✓</i></label>
                      {data.title}
