...
import './CreateStockOrder.scss'

const contract_type = '1';

class CreateStockOrder extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.createStockOrder.DDB.GUID) {
      this.setState({
			  messageDialogOpen: true,
			  messageDialogTitle: '提示',
			  messageDialogMessage: `已创建订单：${nextProps.createStockOrder.DDB.GUID}`
      })
  ...
  handleSupplierChange =(event, index, value) => {
    this.setState({
      ...
      receiveAddress: JSON.stringify(this.props.createStockOrder.receiveAddress[0]),
      isSelectAdress: true});
  }
  ...
  setAddress =(address, index) => (event) => {
   ...
   /*<SelectField
      ...
      >
        {this.props.createStockOrder.authorizeOrganization.map(organization =>
  ...
  renderSupplier() {
    return (
      <SelectDropMenu
        title={'供应商'}
        items={this.props.createStockOrder.authorizeOrganization}
 ...
 /**
  ...
  renderReceiveAddress() {
    return (
      ...
      >
        {this.props.createStockOrder.receiveAddress.map(address =>
  ...
  render() {
    ...
    return (
      <div style={{ height: '100%' }}>
        <StandardForm iconPosition={'-210px 0'} title='创建备货订单'message='...' >
          <StandardFormCardList>
            <StandardFormCard title='订单信息' message='' actions={actions} showStep={false} expanded>
              <div className='WrapContentFlex' style={{padding: '0 16px'}}>
                <div className='col-lg-12 col-md-12 col-sm-12'>
                  ...
                  >
                    <div style={{width:'100%', display: 'inline-block', height: '100%', overflowY: 'auto', marginTop: '16px' }}>
                      <div className='createStockOrder'>
                        ...
                        <div className='orderWidth col-lg-12 col-md-12 col-sm-12'>
                          <div>
                            <LOBSelect
                              ...
                              buyerOrganizationId={this.state.supplier !== '' ? organizationId : null}
        ...
        <ChooseGoodsAuthDialog
          iSNeedFetchApi={ this.state.supplier !== '' ? true : false }
        ...
        >
          {
            this.props.createStockOrder.receiveAddress.map((address, index) => <div
              style={{ cursor: 'pointer' }}
              key={index}
...
<div className='whoName'>
                      <div className='choseSb orderSlect'>
                        <div className='who ' >选择@谁：</div>
                        <div style={{ position: 'relative' }}>
                          <AtSelect organizationId={organizationId} callback={this.handleAtSelectChange} underlineStyle={{ width: '99%' }} inputStyle={{ width: 'inherit' }} />
                        </div>
                      </div>
                      <div className='choseSb'>
                        <div className='who' >填写备注：</div>
                        <div>
                          <AtMessage callback={this.handleAtMessageChange} hintText='' divStyle={{ width: '99%', overflow: 'hidden' }} />
                        </div>
                      </div>
                    </div>
*/
