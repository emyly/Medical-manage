...
import {
  ...
  STOCK_ORDER_ERROR
} from './createConsignmentOrder.js'
...
import { postJSDDAPI } from 'api/DDB'
import { getProductionListAPI } from 'api/SPJCXXB';

export function* watchConsignmentOrderGetProductionList() {
  yield takeEvery(GET_PRODUCTION_LIST, handleAPI(getProductionList))
}

export function* watchConsignmentOrderPostStockOrder() {
  yield takeEvery(POST_STOCK_ORDER, handleAPI(postStockOrder))
}

export function* watchConsignmentOrderGetAuthorizeOrganization() {
  yield takeEvery(GET_AUTHORIZE_ORGANIZATION, handleAPI(getAuthorizeOrganization))
}

export function* watchConsignmentOrderGetReceiveAddress() {
...
export function* postStockOrder(action) {
  try {
    const response = yield call(postJSDDAPI, action.payload);
    yield put({
